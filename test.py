...
# with pdfplumber.open('KOT074 - Kamlesh Mohpal.pdf') as pdf:
#     for txt_pg in pdf.pages:
#         text1 = txt_pg.extract_text()
        
#         try:
#         #     table = txt_pg.extract_table()
#         #     import pandas as pd
...
#     with open ("text.html", "a") as output:
        file1 = file1.read()
        test = re.search('Amt after Due Date:(.*?)<br>',str(file1), re.IGNORECASE).group(1).strip()
        ...
        # # fixed_charges = re.search('Fixed Charges(.*?)<br>',str(file1), re.IGNORECASE).group(1).strip()
        ...
        # CURR_DUE_AMT = re.search('<br>Electricity Tax.*?<br>(.*?)<br>',str(file1), re.IGNORECASE).group(1).strip('.| ')
        # # last_payment = re.search('Last payment of RS(.*?)received',str(file1), re.IGNORECASE).group(1).strip('.| ')
        # # recieved_date = re.search('received on\s+(.*?)\s+',str(file1), re.IGNORECASE).group(1).strip('.| ')
        # # variable3 = re.search('<br>WhatsApp.*?\d{10}(.*?)<br>',str(file1), re.IGNORECASE).group(1).strip('.| ').split()
        # # DUE_AMT = re.search('<br>KWH.*?<br>(.*?)<br>',str(file1), re.IGNORECASE).group(1).strip('.| ')
